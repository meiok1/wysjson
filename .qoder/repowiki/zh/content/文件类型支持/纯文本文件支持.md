...
## 简介
本文件聚焦于 wysJSON 在“纯文本文件”场景下的 JSON 数据提取与编辑能力，重点解释以下内容：
...
## 项目结构
wysJSON 的 VS Code 扩展由“扩展入口 + 解析器 + 模型层 + Webview 编辑器”构成。与纯文本文件支持直接相关的核心模块如下：
...
## 架构总览
下图展示了从用户在任意文本中选择或定位光标，到提取 JSON 字面量并进入 wysJSON 编辑器的关键流程。
...
## 结论
wysJSON 在“纯文本文件支持”方面采用“AST 优先 + 括号扫描回退”的双轨策略：
