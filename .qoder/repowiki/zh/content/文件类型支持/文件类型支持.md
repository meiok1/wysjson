...
## 简介
本文件系统性阐述 JSONOKOK 对不同文件类型的处理策略与实现细节，涵盖以下方面：
...
## 结论
JSONOKOK 通过对 JS/TS、Markdown 与纯文本三类文件的差异化处理，实现了稳健的“选区提取 → AST 转模型 → 模型生成代码”的工作流。其核心优势在于：
...
- 纯文本
  ...
  - 最佳实践：在光标附近存在对象/数组字面量时使用“打开 JSONOKOK”；注意文件过大时的性能影响
