...
## 简介
本文件系统性梳理 wysJSON VS Code 扩展的生命周期钩子与命令注册机制，覆盖扩展激活与停用流程、命令注册与调用、onCommand 事件监听、VS Code API 集成点、扩展点与贡献点、状态管理与资源清理、错误处理与异常恢复、开发模板与最佳实践，以及性能监控与调试建议。目标是帮助开发者快速理解并高效维护该扩展。
...
- 命令注册与调用
  - wysjson.openSelection：默认本地化菜单命令
  - wysjson.openSelectionEnglish：英文菜单命令（受上下文键控制）
- Webview 通信
...
### 生命周期钩子与命令注册
- activate(context)
  - 注册两条命令：wysjson.openSelection 与 wysjson.openSelectionEnglish
  - 将命令实例加入 context.subscriptions，确保卸载时自动释放
  - 设置扩展上下文键 wysjson.lang，用于控制菜单显示语言
  - 记录激活日志
- deactivate()
  - 输出停用日志（当前无显式资源清理）
...
```mermaid
flowchart TD
A["activate(context)"] --> B["注册命令<br/>wysjson.openSelection"]
B --> C["注册命令<br/>wysjson.openSelectionEnglish"]
C --> D["设置上下文键<br/>wysjson.lang"]
D --> E["记录激活日志"]
...
- commands.registerCommand / commands.executeCommand
  - 注册两条命令，并在运行时设置上下文键 wysjson.lang 控制菜单显示
...
### 扩展点与贡献点
- activationEvents
  - onCommand:wysjson.openSelection
  - onCommand:wysjson.openSelectionEnglish
- commands
  - wysjson.openSelection：本地化标题
  - wysjson.openSelectionEnglish：英文标题
- menus.editor/context
  - editor/context 菜单项根据上下文键 wysjson.lang 显示中文或英文
- configuration
  - wysjson.supportedLanguages：控制右键菜单显示的语言列表
...
### 状态管理与资源清理
- 状态管理
  - 用户语言偏好：globalState.wysjson.language
  - 上下文键：wysjson.lang 控制菜单显示
...
- 命令注册
  ...
  - 通过 setContext 控制菜单显示（如 wysjson.lang）
- Webview 通信
...
## 结论
wysJSON 扩展通过清晰的生命周期钩子与命令注册机制，结合稳健的选区提取、AST→模型与模型→源码生成流程，实现了在 VS Code 中对 JS/TS/Markdown/纯文本等文件中 JSON/JS 字面量的可视化编辑与安全回写。其设计强调可逆性与安全性（版本检查、语法校验、codeText 节点保留），并通过上下文键与菜单贡献点实现灵活的本地化体验。未来可在超大 JSON 的懒加载、撤销/重做持久化、代码节点增强等方面持续优化。
