...
## 简介
本文件面向 JSONOKOK VS Code 扩展的使用者与集成者，系统性整理"示例与最佳实践"。内容覆盖：
...
## 架构总览
JSONOKOK 的工作流分为"选区/文档提取 -> AST 转模型 -> Webview 渲染 -> 用户编辑 -> 代码生成 -> 写回源文件"。
...
- **新增**：字符串字段JSON处理问题
  ...
  - 处理：使用字符串编辑模式；避免在JSONOKOK中直接修改JSON字符串内容
  - **新增**：数据库JSON字符串字段问题
    - 对于数据库存储的JSON字符串字段，使用字符串编辑模式
    - 避免在JSONOKOK中直接修改JSON字符串内容
    - 利用WeakMap缓存机制提高解析效率
...
## 结论
JSONOKOK 通过"选区/文档提取 -> AST 转模型 -> Webview 编辑 -> 代码生成 -> 写回"的完整链路，提供了直观、安全、高效的 JSON 编辑体验。结合新增的大型JSON优化和字符串字段JSON处理最佳实践，用户可在不同场景下快速上手并实现稳定集成。
...
- **新增**：字符串字段JSON处理最佳实践
  ...
  - 字段值为JSON字符串：在JSONOKOK中作为字符串编辑，避免误改JSON结构
...
- 复杂嵌套结构编辑
  ...
  - 优先使用选区编辑，减少全量解析
- 特殊格式数据
  - 字段值为 JSON 字符串：在 JSONOKOK 中作为字符串编辑，保持其 JSON 文本形态
...
- **新增**：数据库JSON字符串字段处理
  ...
  - 步骤：在 JSONOKOK 中作为字符串编辑，避免误改 JSON 结构
...
### 快速上手步骤
- 安装扩展后，在 JS/TS 文件中右键选择"JSONOKOK：在 JSONOKOK 编辑器中打开数据"
...
- **新增**：字符串字段JSON处理
  ...
  - 利用延迟解析策略提高编辑效率
  - **新增**：数据库JSON字符串字段处理步骤
    - 使用字符串编辑模式；避免在JSONOKOK中直接修改JSON字符串内容
    - 利用WeakMap缓存机制提高解析效率
