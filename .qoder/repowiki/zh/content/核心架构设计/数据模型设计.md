...
## 简介
本设计文档围绕 wysJSON 的数据模型系统，系统性阐述中间模型（JsonNode）与元数据（SourceInfo）的设计理念、实现细节与使用方式。重点包括：
...
### JsonNode 数据模型
JsonNode 是 wysJSON 的核心数据结构，用于承载可编辑的数据树，并保留原始源码以便安全写回。
...
## 结论
wysJSON 的数据模型系统通过 JsonNode 与 SourceInfo 实现了对复杂 JavaScript 数据的可视化编辑与安全写回。AST→中间模型与中间模型→源码的双向转换保证了编辑体验与源码一致性，配合严格的校验与错误处理机制，提升了系统的可靠性与可用性。通过合理的扩展点与插件生态，未来可进一步支持更多数据类型与编辑能力。

## 附录

### 使用示例与最佳实践
- 在 VS Code 中右键选中对象/数组字面量，或在 Markdown 代码块内选择，即可打开 wysJSON 编辑器
