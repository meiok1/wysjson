...
## 简介
本文件面向 wysJSON VS Code 扩展的“扩展入口与生命周期”主题，系统性梳理扩展的激活与停用流程、命令注册与执行、VS Code 生命周期管理、用户交互处理（右键菜单、光标位置检测、文件类型判断）、扩展上下文资源管理、错误处理与日志记录、调试支持以及初始化最佳实践与性能优化建议。文档同时给出关键流程的可视化图示，帮助读者快速理解代码级实现与运行时行为。
