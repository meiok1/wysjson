...
- WebView 前端：在 VS Code Webview 中承载 JSONOKOK 编辑器界面与交互逻辑。
...
- 入口模块（activate/deactivate）
  - 注册两个命令：JSONOKOK.openSelection 与 JSONOKOK.openSelectionEnglish，并将命令句柄加入 context.subscriptions 实现自动清理。
...
- [media/webview.js:5](file://media/webview.js#L5-L5)
...
Ext->>Cmd : "注册命令 JSONOKOK.openSelection / JSONOKOK.openSelectionEnglish"
VS-->>Ext : "用户触发命令"
...
- 激活（activate）
  ...
  - 注册两条命令：JSONOKOK.openSelection（本地化标题）与 JSONOKOK.openSelectionEnglish（英文标题），并将命令句柄加入 context.subscriptions，确保卸载时自动释放。
  - 输出激活日志，便于调试。
- 停用（deactivate）
  - 输出停用日志，用于生命周期观测。
...
- 命令注册
  ...
  - 区别在于菜单显示：通过上下文键 JSONOKOK.lang 控制本地化与英文菜单项的显示。
- 执行流程
...
- 右键菜单
  ...
  - 菜单可见性由上下文键 JSONOKOK.lang 控制：当语言偏好为英文时显示英文命令，否则显示本地化命令。
- 光标位置检测
...
- [src/extension.ts:148-308](file://src/extension.ts#L148-L308)
...
- 上下文键
  - 使用 setContext("JSONOKOK.lang", effectiveLang) 动态控制菜单显示语言，避免 VS Code 对菜单标签的本地化限制。
- 全局状态
  - 通过 globalState 存储用户语言偏好，实现跨会话持久化。

章节来源
- [src/extension.ts:28-38](file://src/extension.ts#L28-L38)
...
- [src/extension.ts:433-451](file://src/extension.ts#L433-L451)
- [src/extension.ts:470-486](file://src/extension.ts#L470-L486)
- [src/extension.ts:351-356](file://src/extension.ts#L351-L356)
- [src/extension.ts:414-416](file://src/extension.ts#L414-L416)
...
章节来源
- [src/extension.ts:319-347](file://src/extension.ts#L319-L347)
...
- [src/parser/parserOptions.ts:3-35](file://src/parser/parserOptions.ts#L3-L35)
...
## 故障排查指南
- 命令未出现或不可用
  - 检查 activationEvents 是否包含 onCommand:JSONOKOK.openSelection 与 onCommand:JSONOKOK.openSelectionEnglish。
  - 确认菜单配置中 editor/context 的 when 条件与 JSONOKOK.lang 上下文键一致。
- 选区/光标无法提取字面量
...
### 命令与菜单配置要点
- 命令注册
  - JSONOKOK.openSelection：本地化标题，用于显示本地化菜单项。
  - JSONOKOK.openSelectionEnglish：英文标题，用于显示英文菜单项。
- 菜单可见性
  - editor/context 菜单通过 when 条件控制：当 JSONOKOK.lang 为英文时显示英文命令，否则显示本地化命令。
- 语言偏好
  - 用户可在 WebView 中切换语言；扩展通过 globalState 与 setContext 维护偏好。

章节来源
- [package.json:28-51](file://package.json#L28-L51)
- [src/extension.ts:313-326](file://src/extension.ts#L313-L326)
- [src/extension.ts:404-418](file://src/extension.ts#L404-L418)
