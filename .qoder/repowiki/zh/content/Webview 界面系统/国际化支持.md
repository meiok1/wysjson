...
- 扩展入口与语言状态
  - extension.ts：读取用户语言偏好（globalState），设置 JSONOKOK.lang 上下文键，向 webview 注入初始语言参数。
- Webview 运行时翻译
...
VS->>Ext : 用户激活命令
Ext->>Ext : 读取 globalState.JSONOKOK.language
Ext->>VS : 设置上下文键 "JSONOKOK.lang"
...
- 功能要点
  ...
  - extension.ts 注册两个命令：一个带本地化标题，另一个英文标题，通过上下文键 JSONOKOK.lang 控制菜单显示哪条命令。
- 实现细节
  - commands 配置中使用 %command.openSelection% 引用翻译键。
  - menus.editor/context 中根据 JSONOKOK.lang 切换显示英文或本地化命令。
  - extension.ts 在激活时设置上下文键，影响菜单项可见性。

```mermaid
flowchart TD
Start(["扩展激活"]) --> ReadPref["读取用户语言偏好"]
ReadPref --> SetCtx["设置上下文键 JSONOKOK.lang"]
SetCtx --> MenuCheck{"菜单条件判断"}
MenuCheck --> |JSONOKOK.lang == 'en'| ShowEn["显示英文命令"]
...
- 功能要点
  ...
  - **新增** 自动检测功能：根据浏览器语言自动选择语言，支持 zh-CN 和 en 的智能判断。
- 实现细节
  ...
  - **更新** 语言选择器事件处理：当选择 "auto" 时，根据 navigator.language 自动判断语言，zh 开头的语言自动选择 zh-CN，其他语言选择 en。
...
Ext->>Ext : globalState.update("JSONOKOK.language")
Ext->>VS : setContext("JSONOKOK.lang", effective)
...
- 菜单条件控制：通过 JSONOKOK.lang 上下文键控制菜单显示，避免语言冲突。
- **新增** 自动检测验证：确保浏览器语言检测逻辑正确处理各种语言代码格式。
