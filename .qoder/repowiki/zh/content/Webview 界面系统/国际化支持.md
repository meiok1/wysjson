...
## 更新摘要
**所做更改**
- 更新了JSONOKOK支持的多语言环境说明，明确支持中文简体和英文
- 完善了语言切换机制的实现细节，包括自动检测功能
- 增强了语言选择器的样式设计和用户体验
- 优化了语言状态管理和持久化策略
...
## 项目概述
本项目为 VS Code 扩展 JSONOKOK，提供 JSON 数据的可视化表格编辑能力。国际化支持贯穿扩展的命令标题与 UI 文本，采用 VS Code 官方 NLS（非英语本地化）机制与前端运行时翻译系统相结合的方式，实现命令本地化与界面文本本地化。

## 项目结构
国际化相关的关键文件分布如下：
- 扩展元信息与命令本地化：package.json 中的 commands 配置使用 NLS 占位符，实际翻译内容由 package.nls.json 和 package.nls.zh-CN.json 提供。
- 扩展逻辑与语言状态管理：部署方式/vscodeExtension/src/extension.ts 负责语言偏好持久化、上下文键设置与向 webview 传递语言信息。
...
D["extension.ts<br/>语言偏好持久化/上下文键"] --> E["webview.js<br/>运行时翻译与UI更新"]
E --> F["webview.css<br/>语言选择器样式"]
```
...
- [extension.ts:97-112](file://部署方式/vscodeExtension/src/extension.ts#L97-L112)
...
- 扩展入口与语言状态
  - extension.ts：读取用户语言偏好（vscode.env.language），设置 JSONOK.lang 上下文键，向 webview 注入初始语言参数。
- Webview 运行时翻译
...
VS->>Ext : 用户激活命令
Ext->>Ext : 读取 vscode.env.language
Ext->>VS : 设置上下文键 "JSONOK.lang"
...
**图表来源**
- [extension.ts:97-112](file://部署方式/vscodeExtension/src/extension.ts#L97-L112)
- [extension.ts:236-243](file://部署方式/vscodeExtension/src/extension.ts#L236-L243)
- [webview.js:259-286](file://media/webview.js#L259-L286)
- [webview.js:205-252](file://media/webview.js#L205-L252)
- [webview.css:113-127](file://media/webview.css#L113-L127)
...
- 功能要点
  ...
  - extension.ts 注册两个命令：一个带本地化标题，另一个英文标题，通过上下文键 JSONOK.lang 控制菜单显示哪条命令。
- 实现细节
  - commands 配置中使用 %command.openSelection% 引用翻译键。
  - menus.editor/context 中根据 JSONOK.lang 切换显示英文或本地化命令。
  - extension.ts 在激活时设置上下文键，影响菜单项可见性。

```mermaid
flowchart TD
Start(["扩展激活"]) --> ReadPref["读取用户语言偏好"]
ReadPref --> SetCtx["设置上下文键 JSONOK.lang"]
SetCtx --> MenuCheck{"菜单条件判断"}
MenuCheck --> |JSONOK.lang == 'en'| ShowEn["显示英文命令"]
...
- [webview.js:54-187](file://media/webview.js#L54-L187)
- [webview.js:259-286](file://media/webview.js#L259-L286)
- [webview.js:493-513](file://media/webview.js#L493-L513)
- [extension.ts:206-234](file://部署方式/vscodeExtension/src/extension.ts#L206-L234)

**章节来源**
- [webview.js:54-187](file://media/webview.js#L54-L187)
- [webview.js:193-203](file://media/webview.js#L193-L203)
- [webview.js:205-252](file://media/webview.js#L205-L252)
- [webview.js:493-513](file://media/webview.js#L493-L513)
- [extension.ts:206-234](file://部署方式/vscodeExtension/src/extension.ts#L206-L234)
...
- 功能要点
  ...
  - 自动检测功能：根据浏览器语言自动选择语言，支持 zh-CN 和 en 的智能判断。
- 实现细节
  ...
  - 自动检测功能：当选择 "auto" 时，根据 navigator.language 自动判断语言，zh 开头的语言自动选择 zh-CN，其他语言选择 en。
...
User->>WV : 选择语言
alt 选择自动检测
WV->>WV : 检测 navigator.language
WV->>WV : 根据 zh 开头选择 zh-CN
else 选择固定语言
WV->>WV : 使用选择的语言
end
WV->>WV : 更新 currentLanguage 与 document.lang
WV->>Ext : postMessage({ type : "setLanguage", language })
Ext->>Ext : globalState.update("JSONOK.language")
Ext->>VS : setContext("JSONOK.lang", effective)
Ext-->>WV : postMessage({ type : "languageSaved" })
```

**图表来源**
- [webview.js:493-513](file://media/webview.js#L493-L513)
- [extension.ts:319-336](file://部署方式/vscodeExtension/src/extension.ts#L319-L336)
...
## 结论
本项目通过 VS Code NLS 与 webview 运行时翻译系统实现了完整的国际化支持。命令标题与菜单标签由 NLS 管理，界面文本由运行时翻译系统负责，二者协同工作，既满足了扩展层面的本地化需求，又提供了灵活的 UI 文本本地化能力。最新的语言选择器改进包括自动检测功能、增强的样式设计和更好的用户体验，进一步提升了国际化支持的质量和易用性。对于日期、数字、货币等文化适配场景，项目当前未实现，后续可按需扩展。
...
- 菜单条件控制：通过 JSONOK.lang 上下文键控制菜单显示，避免语言冲突。
- 自动检测验证：确保浏览器语言检测逻辑正确处理各种语言代码格式。
