...
- [部署方式/推送两个仓库-git-remote/两个gitremote.md](file://部署方式/推送两个仓库-git-remote/两个gitremote.md)
</cite>

## 更新摘要
**所做更改**
- 新增多仓库部署方式的完整指南，支持推送至两个独立 git 仓库
...
## 简介
本指南面向 JSONOKOK 的全新部署方式，涵盖网页部署和 VS Code 扩展部署的完整流程。该部署方式采用双环境架构，支持同时向两个平台发布产品，并新增了多仓库部署策略：

- **网页部署**：基于 GitHub Pages 的静态网站托管，无需服务器配置
- **VS Code 扩展部署**：完整的扩展打包、签名和市场发布流程
- **多仓库部署**：支持推送至两个独立 git 仓库，实现更灵活的发布策略

**更新** 品牌名称已从 JSONOK 正式升级为 JSONOKOK，体现更完整的品牌识别度。新增 esbuild 构建工具集成，提供更快的构建速度和更优的打包优化。多仓库部署策略允许开发者根据需要选择不同的发布渠道，提高部署灵活性。
...
## 项目结构
仓库采用"双环境分离 + 多仓库部署"的组织方式，每个部署环境都有独立的配置和资源：
...
F --> H["网页部署/<br/>静态网站环境"]
F --> I["推送两个仓库-git-remote/<br/>多仓库部署配置"]
G --> J["package.json<br/>扩展配置"]
G --> K["tsconfig.json<br/>TypeScript配置"]
G --> L["src/extension.ts<br/>扩展入口"]
G --> M["media/<br/>Monaco编辑器资源"]
H --> N["package.json<br/>网站配置"]
H --> O["src/indexMonaco.html<br/>网页入口"]
H --> P["media/<br/>简化资源"]
I --> Q["两个gitremote.md<br/>多仓库配置说明"]
```
...
## 核心组件
JSONOKOK 采用双环境架构，每个环境都有专门的组件，并支持多仓库部署策略：
...
### 多仓库部署策略
- **独立仓库管理**：支持同时向两个独立的 git 仓库推送代码
- **灵活发布渠道**：可根据需要选择不同的发布平台和受众
- **版本同步策略**：确保两个仓库的版本保持一致性和同步性
...
## 架构总览
下图展示 JSONOKOK 双环境部署的端到端流程，包括多仓库部署策略：
...
participant MultiRepo as "多仓库部署"
participant Test as "测试环境"
Dev->>Build : 触发构建
Build->>ESBuild : esbuild快速打包
ESBuild->>VSCE : 生成优化的扩展包
Build->>GH : 部署静态网站
Build->>MultiRepo : 推送到多个仓库
Build->>Test : 运行自动化测试
VSCE-->>Dev : .vsix扩展包
GH-->>Dev : 在线访问链接
MultiRepo-->>Dev : 多仓库发布状态
Test-->>Dev : 测试报告和反馈
```
...
#### 多仓库部署配置
- **仓库配置**：`部署方式/推送两个仓库-git-remote/两个gitremote.md` 描述多仓库设置
- **远程仓库管理**：支持同时配置多个 git 远程仓库
- **推送策略**：可选择推送至一个或多个仓库
...
CheckEnv --> |多仓库| RepoConfig["加载仓库配置"]
...
RepoConfig --> MultiPush["配置多仓库推送"]
MultiPush --> RepoBuild["生成多仓库资源"]
...
- [部署方式/prepare_deploy.ps1](file://部署方式/prepare_deploy.ps1)
- [部署方式/consolidate_deploy.ps1](file://部署方式/consolidate_deploy.ps1)
- [deploy_site.ps1](file://部署方式/deploy_site.ps1)

### 多仓库部署策略

#### 配置说明
- **配置文件**：`部署方式/推送两个仓库-git-remote/两个gitremote.md` 提供详细的多仓库配置指南
- **仓库管理**：支持同时配置多个 git 远程仓库，实现代码的多点分发
- **推送策略**：可选择推送至一个或多个仓库，满足不同的发布需求

#### 部署流程
- **仓库初始化**：配置多个 git 远程仓库，建立推送关系
- **构建优化**：针对多仓库部署进行资源优化和版本管理
- **批量推送**：一次性将构建产物推送到所有配置的仓库
- **状态监控**：跟踪每个仓库的推送状态和部署结果

#### 最佳实践
- **版本同步**：确保所有仓库的版本号保持一致
- **冲突处理**：合理处理多仓库间的代码冲突和分支管理
- **回滚策略**：建立多仓库的回滚机制，确保发布安全
- **权限管理**：为不同仓库配置相应的访问权限和部署权限
...
MultiRepo["多仓库部署<br/>部署方式/推送两个仓库-git-remote"] --> GitTools["Git工具<br/>远程仓库管理"]
MultiRepo --> Sync["同步策略<br/>版本和状态管理"]
```
...
#### 多仓库部署问题
- **推送失败**：检查 git 远程仓库配置和访问权限
- **版本不一致**：验证各仓库的版本同步状态
- **冲突解决**：处理多仓库间的代码冲突和分支差异
- **同步延迟**：监控多仓库同步状态，及时发现和解决问题
...
## 结论
JSONOKOK 的全新部署方式提供了灵活的双环境架构，既满足 VS Code 扩展的深度集成需求，又支持网页部署的广泛访问。通过统一的核心代码库和专门的部署配置，实现了高效的开发、测试和发布流程。新增的 esbuild 构建工具显著提升了构建速度和优化效果，自动化工具和脚本确保了部署的一致性和可靠性。多仓库部署策略进一步增强了发布的灵活性，允许开发者根据需要选择不同的发布渠道，为用户提供了优质的使用体验。
